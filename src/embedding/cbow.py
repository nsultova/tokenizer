...
def cbow_trainingdata_debug(corpus, context_size, X, Y):
    [idx2word, word2idx] = corpus.translators()
    for sample in [[[idx2word[x] for x in xs], idx2word[y]] for [xs,y] in list(zip(X,Y))[196:200]]:
...
def cbow_trainingdata_for_corpus(corpus, context_size=2):
    ...
    >>> cbow_trainingdata_debug(corpus, context_size, X, Y)
